...
// --- CORRECCIÓN PROBLEMA 1: Exportar función para redimensionar ---
...
return  `<tr class="border-b border-gray-800 text-red-400"> <td class="py-3 px-3">${t.date}</td> <td class="py-3 px-3">${escapeHTML(t.description)}</td> <td class="py-3 px-3 italic">Cuenta no encontrada (ID: ${t.accountId})</td> <td class="py-3 px-3">${escapeHTML(t.category)}</td> <td class="py-3 px-3">${escapeHTML(t.part)}</td> <td class="py-3 px-3 text-right">-</td> <td class="py-3 px-3 text-right">-</td> <td class="py-3 px-3"></td> </tr>`;
}
...
return  `<div class="card p-6 rounded-xl flex flex-col"> <div class="flex justify-between items-start mb-2"> <h3 class="font-semibold text-lg">${escapeHTML(account.name)}</h3> <div>${account.logoHtml || '<i data-lucide="wallet" class="text-gray-500"></i>'}</div> </div> <div class="mt-auto"> <p class="text-gray-400 text-sm">Saldo Actual</p> <p class="text-4xl font-bold kpi-value mt-2">${formatCurrency(account.balance, account.currency)}</p> </div> </div>`;
}
...
return  `<tr class="border-b border-gray-800 text-red-400"> <td class="py-3 px-3">${t.date}</td> <td class="py-2 px-3 italic">Datos incompletos</td> <td class="py-2 px-3 italic">Cuenta no encontrada (ID: ${t.accountId})</td> <td class="py-2 px-3 text-right">-</td> <td class="py-2 px-3 text-center"></td> </tr>`;
}
...
console.log('[renderTransactions] Rendering with accounts:', accounts);
// Añadir chequeos robustos
...
console.log('[renderAccountsTab] Rendering with accounts:', accounts);
if (!accountsGrid) return;
accountsGrid.innerHTML = accounts.map(createAccountCard).join('');

if (typeof lucide !== 'undefined' && lucide.createIcons && accountsGrid.querySelector('i[data-lucide]')) {
     lucide.createIcons({ nodes: accountsGrid.querySelectorAll('i[data-lucide]') });
}
...
console.log('[renderBalanceLegendAndChart] Rendering with accounts:', accounts);
// Esta línea sobraba: if (!totalsContainer) return;
...
div.innerHTML =  `<div> <span class="font-semibold">${escapeHTML(asset.name)}</span> <span class="text-gray-400 text-xs">(${escapeHTML(asset.category)})</span> </div> <button class="delete-investment-asset-btn p-1 text-red-400 hover:text-red-300" data-id="${asset.id}"> <i data-lucide="trash-2" class="w-4 h-4"></i> </button>`;
...
const printWindow = window.open('', '', 'height=800,width=800');
        // Usar template literal para construir el HTML a escribir en la nueva ventana
...
export function renderAll() {
    const state = getState();
    // Asegurar que el estado y las cuentas estén cargados antes de renderizar
    if (!state || !state.accounts) {
        console.warn("[renderAll] Estado o cuentas no disponibles aún. Esperando...");
        return;
    }

    console.log("[renderAll] Starting render cycle with state:", state);
    ...
    if (visiblePage) {
        ...
        console.log(`[renderAll] Rendering page: ${pageId}`);
        switch (pageId) {
            case 'inicio':
    ...
    // Repoblar selects dependientes de estado
    populateSelects();

    // Recrear íconos de Lucide en el contenido principal si los hay
    if (typeof lucide !== 'undefined' && typeof lucide.createIcons === 'function') {
        try {
            const iconsToCreate = elements.mainContent.querySelectorAll('i[data-lucide]');
            if (iconsToCreate.length > 0) {
                console.log(`[renderAll] Recreating ${iconsToCreate.length} Lucide icons.`);
